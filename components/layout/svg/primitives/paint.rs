...
pub(crate) fn build_fill(computed: &ComputedValues) -> Option<usvg::Fill> {
    ...
    let mut fill = usvg::Fill::default();
    fill.set_paint(paint);
    fill.set_opacity(usvg::Opacity::new(opacity).unwrap_or(usvg::Opacity::ONE));
    fill.set_rule(rule);
    Some(fill)
}
...
pub(crate) fn build_stroke(computed: &ComputedValues, diagonal: f32) -> Option<usvg::Stroke> {
    ...
    stroke.set_paint(paint);
    stroke.set_width(usvg::StrokeWidth::new(width).unwrap_or(usvg::StrokeWidth::new(1.0).unwrap()));
    // `stroke-opacity` multiplies the alpha already carried by the `stroke`
    // color, mirroring the fill path above.
    stroke.set_opacity(match inherited.stroke_opacity {
    ...
    stroke.set_linecap(match inherited.stroke_linecap {
    ...
    stroke.set_linejoin(match inherited.stroke_linejoin {
    ...
    stroke.set_miterlimit(usvg::StrokeMiterlimit::new(inherited.stroke_miterlimit.0));
    if let SVGStrokeDashArray::Values(vs) = &inherited.stroke_dasharray {
        if !vs.is_empty() {
            ...
            stroke.set_dasharray(Some(dasharray));
        }
    }
    stroke.set_dashoffset(match &inherited.stroke_dashoffset {
