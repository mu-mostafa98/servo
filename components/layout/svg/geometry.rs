...
pub(crate) fn build_text(node: ServoLayoutNode, get: &dyn Fn(&str) -> Option<String>, fs: f32) -> Option<TextSpan> {
    ...
    Some(TextSpan { text, x, y, dx, dy, text_anchor, glyphs: vec![], font_instance_key: None })
}

/// Parse a space/comma-separated list of lengths from an attribute.
fn parse_length_list(
    name: &str,
    ...
    val.split(|c: char| c == ',' || c.is_ascii_whitespace())
        .filter_map(|s| {
            let t = s.trim();
            if t.is_empty() { None } else { parse_length_simple(t, fs) }
        })
        .collect()
}
