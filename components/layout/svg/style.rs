    ...
    fn from_computed_values(values: &style::properties::ComputedValues) -> Option<Self> {
        ...
        match paint {
            ResolvedPaint::Color(color) => Some(FillParams {
                color: Some(color), paint_server: None, opacity, fill_rule,
            }),
            ResolvedPaint::PaintServer(id) => Some(FillParams {
                color: None, paint_server: Some(PaintServer::Gradient(id)), opacity, fill_rule,
            }),
            ResolvedPaint::None => {
                if matches!(inherited_svg.fill.kind, SVGPaintKind::None) {
                    ...
                    Some(FillParams {
                        ...
                        paint_server: None, opacity, fill_rule,
                    })
                }
            },
        }
    ...
    fn from_computed_values(values: &style::properties::ComputedValues) -> Option<Self> {
        ...
        let dash_array = match &inherited_svg.stroke_dasharray {
            SVGStrokeDashArray::Values(vs) => {
                if vs.is_empty() { None } else {
                    Some(vs.iter().map(|v| v.0.to_length().map(|l| l.px()).unwrap_or(0.0)).collect())
                }
            },
            _ => None,
        };
        ...
        match paint {
            ResolvedPaint::Color(color) => Some(StrokeParams {
                color: Some(color), paint_server: None, opacity, width,
                line_cap, line_join, miter_limit, dash_array, dash_offset,
            }),
            ResolvedPaint::PaintServer(id) => Some(StrokeParams {
                color: None, paint_server: Some(PaintServer::Gradient(id)), opacity, width,
                line_cap, line_join, miter_limit, dash_array, dash_offset,
            }),
...
impl FromComputedValues for NodeStyle {
    fn from_computed_values(values: &style::properties::ComputedValues) -> Option<Self> {
        ...
        { Display::None } else { Display::Inline };
        ...
        let mask_ref = values.get_svg().mask_image.0.first().and_then(|img| match img {
            ComputedImage::Url(style::url::ComputedUrl::Valid(u)) => u.fragment().map(|s| s.to_owned()),
        ...
        let effects = match (clip_path_ref, mask_ref) {
            (None, None) => None,
            (clip, mask) => Some(NodeEffects { clip_path: clip, mask, filter: None }),
        };
        let sr = values.get_inherited_svg().shape_rendering;
        let shape_rendering_hint = match sr {
            style::computed_values::shape_rendering::T::Optimizespeed => Some(ShapeRendering::OptimizeSpeed),
            style::computed_values::shape_rendering::T::Crispedges => Some(ShapeRendering::CrispEdges),
            style::computed_values::shape_rendering::T::Geometricprecision => Some(ShapeRendering::GeometricPrecision),
            _ => None,
        };
...
fn apply_stroke_presentation_attrs(element: &ServoLayoutElement, style: &mut NodeStyle) {
    ...
    if stroke_value.eq_ignore_ascii_case("none") { style.stroke = None; return; }
    ...
    let stroke = style.stroke.get_or_insert_with(|| StrokeParams {
        ...
        dash_array: None, dash_offset: 0.0,
    });

    match PaintServer::from_attr(&stroke_value) {
        Some(PaintServer::Solid(c)) => { stroke.color = Some(c); stroke.paint_server = None; },
        Some(PaintServer::Gradient(id)) => { stroke.color = None; stroke.paint_server = Some(PaintServer::Gradient(id)); },
        Some(PaintServer::Pattern(_)) => { stroke.color = None; stroke.paint_server = None; },
        None => { stroke.color = None; stroke.paint_server = None; },
    }
    if let Some(v) = read_attr("stroke-width") { stroke.width = v.trim_end_matches("px").parse::<f32>().unwrap_or(1.0).max(0.0); }
    if let Some(v) = read_attr("stroke-opacity") { stroke.opacity = v.parse::<f32>().unwrap_or(1.0).clamp(0.0, 1.0); }
    if let Some(v) = read_attr("stroke-linecap") { stroke.line_cap = match v.trim() { "round" => LineCap::Round, "square" => LineCap::Square, _ => LineCap::Butt }; }
    if let Some(v) = read_attr("stroke-linejoin") { stroke.line_join = match v.trim() { "round" => LineJoin::Round, "bevel" => LineJoin::Bevel, _ => LineJoin::Miter }; }
    if let Some(v) = read_attr("stroke-miterlimit") { if let Ok(ml) = v.parse::<f32>() { stroke.miter_limit = ml; } }
    if let Some(v) = read_attr("stroke-dasharray") {
        if v.eq_ignore_ascii_case("none") { stroke.dash_array = None; } else {
            let dashes: Vec<f32> = v.split(|c| c == ',' || c == ' ').filter_map(|s| { let t = s.trim(); if t.is_empty() { None } else { t.parse::<f32>().ok() } }).collect();
            stroke.dash_array = if dashes.is_empty() { None } else { Some(dashes) };
        }
    }
    if let Some(v) = read_attr("stroke-dashoffset") { stroke.dash_offset = v.trim_end_matches("px").parse::<f32>().unwrap_or(0.0); }
}
...
fn apply_fill_presentation_attrs(element: &ServoLayoutElement, style: &mut NodeStyle) {
    ...
    if fill_value.eq_ignore_ascii_case("none") { style.fill = None; return; }
    ...
    match PaintServer::from_attr(&fill_value) {
        Some(PaintServer::Solid(c)) => { fill.color = Some(c); fill.paint_server = None; },
        Some(PaintServer::Gradient(id)) => { fill.color = None; fill.paint_server = Some(PaintServer::Gradient(id)); },
        Some(PaintServer::Pattern(id)) => { fill.color = None; fill.paint_server = Some(PaintServer::Pattern(id)); },
        None => { fill.color = None; fill.paint_server = None; },
    }
    if let Some(v) = read_attr("fill-opacity") { fill.opacity = v.parse::<f32>().unwrap_or(1.0).clamp(0.0, 1.0); }
    if let Some(v) = read_attr("fill-rule") { fill.fill_rule = match v.trim() { "evenodd" | "even-odd" => FillRule::EvenOdd, _ => FillRule::NonZero }; }
}
...
fn apply_render_hints_from_attrs(element: &ServoLayoutElement, style: &mut NodeStyle) {
    let hints = style.render_hints.get_or_insert_with(|| RenderHints {
        vector_effect: None, shape_rendering: None,
        color_rendering: None, color_interpolation: None,
        text_rendering: None, image_rendering: None, paint_order: None,
    });
...
fn apply_presentation_attrs(element: &ServoLayoutElement, style: &mut NodeStyle) {
    ...
    if let Some(v) = read_attr_or_inline("visibility") {
        match v.trim() { "hidden" | "collapse" => style.visibility = Visibility::Hidden, _ => {} }
    }
    // opacity
    if let Some(v) = read_attr_or_inline("opacity") {
        if let Ok(op) = v.parse::<f32>() { style.opacity = op.clamp(0.0, 1.0); }
    }
    // display
    if let Some(v) = read_attr_or_inline("display") {
        if v.trim().eq_ignore_ascii_case("none") { style.display = Display::None; }
    }
...
fn apply_filter_attribute(element: &ServoLayoutElement, style: &mut NodeStyle) {
    let filter_ref = get_attr(element, "filter").as_deref().and_then(extract_url_fragment);
    if let Some(filter_id) = filter_ref {
        ...
        style.effects = Some(NodeEffects {
            clip_path: existing.clip_path, mask: existing.mask, filter: Some(filter_id),
        });
    }
}
