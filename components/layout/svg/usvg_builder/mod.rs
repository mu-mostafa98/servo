...
///
/// A shape normally produces a single node (or a carrying group when it has an
/// opacity below 1.0). Callers must therefore push every returned node.
pub(crate) fn build_usvg_node<'a, 'dom>(
    ...
    if is_group_element(ty) {
        return build_group(node, ctx).into_iter().collect();
    }
