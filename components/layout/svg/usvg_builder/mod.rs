...
//!
//! The builders whose output maps 1:1 onto a single `usvg` node type live in their
//! own submodules ([`path`], [`image`], [`text`]); the marker machinery lives in
//! [`marker`] (it decorates shapes with sibling groups). Everything else that
//! produces a `usvg::Group` — plus the shared orchestration and context — lives
//! here in `mod.rs`.
