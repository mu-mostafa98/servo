...
fn apply_css_property(style: &mut NodeStyle, prop: &str, value: &str) {
    match prop {
                ...
                match ps {
                    PaintServer::Solid(c) => {
                        style.stroke = Some(StrokeParams {
                            color: Some(c), paint_server: None,
        ...
        "stroke-linecap" => {
            let lc = match value { "round" => LineCap::Round, "square" => LineCap::Square, _ => LineCap::Butt };
            if let Some(ref mut s) = style.stroke { s.line_cap = lc; }
        },
        "stroke-linejoin" => {
            let lj = match value { "round" => LineJoin::Round, "bevel" => LineJoin::Bevel, _ => LineJoin::Miter };
            if let Some(ref mut s) = style.stroke { s.line_join = lj; }
        },
        "stroke-dasharray" => {
            if value != "none" {
                let dashes: Vec<f32> = value.split(',').filter_map(|v| v.trim().parse::<f32>().ok()).collect();
                if !dashes.is_empty() {
        ...
        "opacity" => {
            if let Ok(op) = value.parse::<f32>() { style.opacity = op.clamp(0.0, 1.0); }
        },
        "visibility" => {
            style.visibility = match value { "hidden" | "collapse" => Visibility::Hidden, _ => Visibility::Visible };
        },
        _ => {},
    }
}
