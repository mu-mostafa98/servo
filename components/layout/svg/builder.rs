...
impl<'dom, 'a> SvgRenderTreeBuilder<'dom, 'a> {
    ...
    fn build_render_node(
        ...
        Some(SvgRenderNode { id, tag, style, transforms, children })
    }
}
...
fn build_text_node(
    ...
    let id = extract_id(&element);
    Some(SvgRenderNode { id, tag, style, transforms, children: vec![] })
}
...
fn shape_text_span(span: &mut TextSpan, node: ServoLayoutNode, context: &LayoutContext) {
    ...
    if span.text.is_empty() { return; }

    // Build a font group from the element's computed style.
    let Some(font_group) = (|| {
        let element = node.as_element()?;
        if !element.style_data().is_some() { return None; }
    ...
    for (i, &ch) in chars.iter().enumerate() {
        ...
        if let Some(font) = font_group.find_by_codepoint(&*context.font_context, ch, next_ch, language) {
            ...
            if let Some(glyph_id) = font.glyph_index(ch) {
                ...
                glyphs.push(ShapedGlyph { x, y: 0.0, advance, glyph_id: glyph_id as u32, character: ch });
        ...
        let advance = 8.0f32;
        glyphs.push(ShapedGlyph { x, y: 0.0, advance, glyph_id: 0, character: ch });
        x += advance + span.dx.get(i).copied().unwrap_or(0.0);
    }
    span.glyphs = glyphs;
...
fn resolve_use_children<'dom>(
    ...
    if resolving.contains(&ref_id) { return vec![]; }
...
fn build_image_tag(element: &ServoLayoutElement) -> Option<SvgImage> {
    ...
    Some(SvgImage { x, y, width: w, height: h, href })
}
