...
fn resolve_linear_gradient_coords(
    ...
    match lg.units {
        GradientUnits::ObjectBoundingBox => (
            bx + lg.x1.to_object_bbox() * bw, by + lg.y1.to_object_bbox() * bh,
            bx + lg.x2.to_object_bbox() * bw, by + lg.y2.to_object_bbox() * bh,
        ),
        GradientUnits::UserSpaceOnUse => (
            ctx.svg_origin.x + lg.x1.to_user_space(bw), ctx.svg_origin.y + lg.y1.to_user_space(bh),
            ctx.svg_origin.x + lg.x2.to_user_space(bw), ctx.svg_origin.y + lg.y2.to_user_space(bh),
        ),
    }
}
...
fn build_linear_fill_style<'a>(
    ...
    gx1: f32, gy1: f32, gx2: f32, gy2: f32,
    opacity: f32, ctx: &RenderContext,
) -> FillStyle<'a> {
    FillStyle::LinearGradient {
        stops: &lg.stops, gx1, gy1, gx2, gy2, opacity,
...
fn build_radial_fill_style<'a>(
    ...
    fx: f32, fy: f32, radius: f32,
    opacity: f32, ctx: &RenderContext,
) -> FillStyle<'a> {
    FillStyle::RadialGradient {
        stops: &rg.stops, fx, fy, r2: radius * radius, opacity,
        color_interpolation: color_interpolation_hint(ctx),
