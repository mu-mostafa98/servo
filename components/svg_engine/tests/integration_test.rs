...
use svg_engine::{SvgImage, TextAnchor, TextSpan, SvgTag};
...
#[test]
fn rect_data() {
    let r = Rectangle { x: 0.0, y: 0.0, width: 100.0, height: 50.0, rx: None, ry: None };
...
#[test]
fn rect_with_radius() {
    let r = Rectangle { x: 10.0, y: 20.0, width: 200.0, height: 100.0, rx: Some(10.0), ry: Some(5.0) };
...
fn rect_rx_inherits_ry_and_vice_versa() {
    // rx only: ry = rx
    let r = Rectangle { x: 0.0, y: 0.0, width: 100.0, height: 100.0, rx: Some(10.0), ry: None };
    assert_eq!(r.rx, Some(10.0));
    // ry only: rx = ry
    let r2 = Rectangle { x: 0.0, y: 0.0, width: 100.0, height: 100.0, rx: None, ry: Some(15.0) };
    assert_eq!(r2.ry, Some(15.0));
}

#[test]
fn circle_data() {
    let c = Circle { cx: 50.0, cy: 50.0, r: 30.0 };
    assert_eq!(c.cx, 50.0);
    assert_eq!(c.cy, 50.0);
    assert_eq!(c.r, 30.0);
}
...
#[test]
fn ellipse_data() {
    let e = Ellipse { cx: 100.0, cy: 80.0, rx: 60.0, ry: 40.0 };
    assert_eq!(e.rx, 60.0);
    assert_eq!(e.ry, 40.0);
}
...
#[test]
fn line_data() {
    let l = Line { x1: 0.0, y1: 0.0, x2: 100.0, y2: 100.0 };
...
#[test]
fn polyline_data() {
    let pts = vec![kurbo::Point::new(0.0, 0.0), kurbo::Point::new(50.0, 100.0), kurbo::Point::new(100.0, 0.0)];
...
#[test]
fn polygon_data() {
    let pts = vec![kurbo::Point::new(0.0, 0.0), kurbo::Point::new(100.0, 0.0), kurbo::Point::new(50.0, 100.0)];
...
fn shape_enum_all_variants_constructible() {
    let _rect = Shape::Rect(Rectangle { x: 0.0, y: 0.0, width: 10.0, height: 10.0, rx: None, ry: None });
    let _circle = Shape::Circle(Circle { cx: 5.0, cy: 5.0, r: 5.0 });
    let _ellipse = Shape::Ellipse(Ellipse { cx: 5.0, cy: 5.0, rx: 5.0, ry: 3.0 });
    let _line = Shape::Line(Line { x1: 0.0, y1: 0.0, x2: 10.0, y2: 10.0 });
    // Text and Image are in SvgTag, not Shape
    let _text_tag = SvgTag::Text(TextSpan { text: "Hi".into(), x: 10.0, y: 20.0, dx: vec![], dy: vec![], text_anchor: TextAnchor::Start, glyphs: vec![], font_instance_key: None });
    let _image_tag = SvgTag::Image(SvgImage { x: 0.0, y: 0.0, width: 100.0, height: 100.0, href: Some("test.png".into()) });
    // All construct without panic.
}

#[test]
fn text_span_data() {
    let t = TextSpan { text: "Hello SVG".into(), x: 10.0, y: 30.0, dx: vec![], dy: vec![], text_anchor: TextAnchor::Start, glyphs: vec![], font_instance_key: None };
...
fn text_span_with_dx_dy() {
    let t = TextSpan { text: "AB".into(), x: 0.0, y: 0.0, dx: vec![5.0, 10.0], dy: vec![0.0, 3.0], text_anchor: TextAnchor::Start, glyphs: vec![], font_instance_key: None };
...
#[test]
fn svg_image_data() {
    let img = SvgImage { x: 10.0, y: 20.0, width: 300.0, height: 200.0, href: Some("image.png".into()) };
...
fn svg_image_no_href() {
    let img = SvgImage { x: 0.0, y: 0.0, width: 50.0, height: 50.0, href: None };
...
fn line_no_fill_geometry_by_spec() {
    ...
    let line = Shape::Line(Line { x1: 0.0, y1: 0.0, x2: 10.0, y2: 10.0 });
    assert!(matches!(line, Shape::Line(_)));
}

#[test]
fn rect_has_fill_and_stroke_geometry() {
    let rect = Shape::Rect(Rectangle { x: 0.0, y: 0.0, width: 100.0, height: 100.0, rx: None, ry: None });
...
fn node_effects_default_empty() {
    let effects = NodeEffects { clip_path: None, mask: None, filter: None };
...
fn node_effects_with_clip_path() {
    let effects = NodeEffects { clip_path: Some("c1".into()), mask: None, filter: None };
...
fn svg_tag_shape_and_container() {
    let shape_tag = SvgTag::Shape(Shape::Rect(Rectangle { x: 0.0, y: 0.0, width: 10.0, height: 10.0, rx: None, ry: None }));
...
fn viewport_info_defaults() {
    let vp = ViewportInfo { width: 300.0, height: 150.0, view_box: None, overflow_visible: false, aspect_ratio: None };
...
fn viewport_with_viewbox() {
    let vp = ViewportInfo {
        ...
        view_box: Some(ViewBox { min_x: 0.0, min_y: 0.0, width: 100.0, height: 100.0 }),
        overflow_visible: false, aspect_ratio: None,
    };
...
fn aspect_align_all_10_variants_exist() {
    let aligns = [
        AspectAlign::None,
        AspectAlign::XMinYMin, AspectAlign::XMidYMin, AspectAlign::XMaxYMin,
        AspectAlign::XMinYMid, AspectAlign::XMidYMid, AspectAlign::XMaxYMid,
        AspectAlign::XMinYMax, AspectAlign::XMidYMax, AspectAlign::XMaxYMax,
    ];
...
fn parse_aspect_ratio_all_valid_aligns() {
    for align in ["xMinYMin","xMidYMin","xMaxYMin","xMinYMid","xMidYMid","xMaxYMid","xMinYMax","xMidYMax","xMaxYMax"] {
        let ar = parse_aspect_ratio(align);
        assert!(!matches!(ar.align, AspectAlign::None), "Align should not be None for {align}");
    }
}
...
fn viewbox_valid() {
    ...
    assert_eq!((vb.min_x, vb.min_y, vb.width, vb.height), (0.0, 0.0, 200.0, 200.0));
}
...
fn paint_server_solid_named_color() {
    for color in ["red", "blue", "green", "black", "white", "yellow", "purple", "orange"] {
...
fn gradient_stop_ordering_by_offset() {
    let stops = vec![
        GradientStop { offset: 0.5, color: svgtypes::Color::new_rgb(128, 128, 128) },
        GradientStop { offset: 0.0, color: svgtypes::Color::new_rgb(0, 0, 0) },
        GradientStop { offset: 1.0, color: svgtypes::Color::new_rgb(255, 255, 255) },
    ];
...
fn gradient_units_both_variants() {
    assert!(matches!(GradientUnits::ObjectBoundingBox, GradientUnits::ObjectBoundingBox));
    assert!(matches!(GradientUnits::UserSpaceOnUse, GradientUnits::UserSpaceOnUse));
}
...
fn linear_gradient_default_x2() {
    ...
    let lg = LinearGradient {
        id: "g".into(), x1: GradientLength::Number(0.0), y1: GradientLength::Number(0.0),
        x2: GradientLength::Percentage(100.0), y2: GradientLength::Number(0.0),
        units: GradientUnits::ObjectBoundingBox, stops: vec![], transform: vec![],
...
fn radial_gradient_default_center() {
    let rg = RadialGradient {
        id: "g".into(),
        cx: GradientLength::Percentage(50.0), cy: GradientLength::Percentage(50.0),
        r: GradientLength::Percentage(50.0),
        fx: GradientLength::Percentage(50.0), fy: GradientLength::Percentage(50.0),
        units: GradientUnits::ObjectBoundingBox, stops: vec![], transform: vec![],
        spread_method: SpreadMethod::Pad,
    };
...
fn clip_path_units_both_variants() {
    assert!(matches!(ClipPathUnits::ObjectBoundingBox, ClipPathUnits::ObjectBoundingBox));
    assert!(matches!(ClipPathUnits::UserSpaceOnUse, ClipPathUnits::UserSpaceOnUse));
}

#[test]
fn clip_path_def_non_empty_shapes() {
    let shapes = vec![Shape::Rect(Rectangle { x: 0.0, y: 0.0, width: 100.0, height: 100.0, rx: None, ry: None })];
    let def = ClipPathDef { shapes, clip_path_units: ClipPathUnits::UserSpaceOnUse };
    assert_eq!(def.shapes.len(), 1);
}

#[test]
fn mask_def_with_shapes_and_styles() {
    let rect = Shape::Rect(Rectangle { x: 0.0, y: 0.0, width: 10.0, height: 10.0, rx: None, ry: None });
...
fn filter_def_all_primitive_variants() {
    ...
    let def = FilterDef { primitives, x: -0.1, y: -0.1, width: 1.2, height: 1.2 };
    assert_eq!(def.primitives.len(), 3);
    assert!(matches!(def.primitives[0], FilterPrimitive::GaussianBlur(2.0, 2.0)));
    assert!(matches!(def.primitives[1], FilterPrimitive::DropShadow(_, _, _, _, _, _, _)));
...
fn pattern_def_basic() {
    ...
    let shapes = vec![(rect, NodeStyle::default())];
    let def = PatternDef {
        width: 20.0, height: 20.0, x: 0.0, y: 0.0,
...
fn pattern_units_variants() {
    assert!(matches!(PatternUnits::ObjectBoundingBox, PatternUnits::ObjectBoundingBox));
    assert!(matches!(PatternUnits::UserSpaceOnUse, PatternUnits::UserSpaceOnUse));
}

#[test]
fn pattern_content_units_variants() {
    assert!(matches!(PatternContentUnits::ObjectBoundingBox, PatternContentUnits::ObjectBoundingBox));
    assert!(matches!(PatternContentUnits::UserSpaceOnUse, PatternContentUnits::UserSpaceOnUse));
}
...
fn transform_op_matrix() {
    ...
    assert!(matches!(op, TransformOp::Matrix([1.0, 0.0, 0.0, 1.0, 50.0, 50.0])));
}
...
fn color_rendering_variants_exist() {
    ...
    assert!(matches!(ColorRendering::OptimizeSpeed, ColorRendering::OptimizeSpeed));
    assert!(matches!(ColorRendering::OptimizeQuality, ColorRendering::OptimizeQuality));
}
...
fn color_interpolation_variants_exist() {
    ...
    assert!(matches!(ColorInterpolation::LinearRGB, ColorInterpolation::LinearRGB));
}
...
fn color_interpolation_linear_rgb_gradient_math() {
    ...
    // sRGB midpoint
    let srgb_mid = svg_engine::color_at_t_with_space(
        &stops, 0.5, ColorInterpolation::Srgb,
    );
    // Linear RGB midpoint
    let linear_mid = svg_engine::color_at_t_with_space(
        &stops, 0.5, ColorInterpolation::LinearRGB,
    );
    ...
    assert!((srgb_mid.r - linear_mid.r).abs() > 0.0 || (srgb_mid.b - linear_mid.b).abs() > 0.0,
...
fn render_hints_with_color_interpolation() {
    ...
    assert!(matches!(hints.color_interpolation, Some(ColorInterpolation::LinearRGB)));
}
...
fn render_hints_with_color_rendering_optimize_quality() {
    ...
    assert!(matches!(hints.color_rendering, Some(ColorRendering::OptimizeQuality)));
}
...
fn paintserver_fixup_converts_gradient_to_pattern() {
    ...
    patterns.insert("myPat".to_owned(), PatternDef {
        width: 10.0, height: 10.0, x: 0.0, y: 0.0,
        pattern_units: PatternUnits::UserSpaceOnUse,
        pattern_content_units: PatternContentUnits::UserSpaceOnUse,
        shapes: vec![],
    });
...
fn render_tree_with_gradient_def() {
    ...
    let grad = GradientDef::Linear(LinearGradient {
        id: "g1".into(),
        x1: GradientLength::Number(0.0), y1: GradientLength::Number(0.0),
        x2: GradientLength::Percentage(100.0), y2: GradientLength::Number(0.0),
        units: GradientUnits::ObjectBoundingBox, stops: vec![], transform: vec![],
        spread_method: SpreadMethod::Pad,
    });
...
fn render_hints_non_scaling_stroke() {
    ...
    assert!(matches!(hints.vector_effect, Some(VectorEffect::NonScalingStroke)));
}

#[test]
fn shape_rendering_all_variants() {
    assert!(matches!(ShapeRendering::Auto, ShapeRendering::Auto));
    assert!(matches!(ShapeRendering::OptimizeSpeed, ShapeRendering::OptimizeSpeed));
    assert!(matches!(ShapeRendering::CrispEdges, ShapeRendering::CrispEdges));
    assert!(matches!(ShapeRendering::GeometricPrecision, ShapeRendering::GeometricPrecision));
}
...
fn make_simple_tree() -> SvgRenderTree {
    let child1 = SvgRenderNode {
        ...
        tag: SvgTag::Shape(Shape::Circle(Circle { cx: 10.0, cy: 10.0, r: 5.0 })),
        style: NodeStyle::default(),
        transforms: vec![],
        children: vec![],
    };
    let child2 = SvgRenderNode {
        ...
        tag: SvgTag::Shape(Shape::Rect(Rectangle { x: 0.0, y: 0.0, width: 20.0, height: 20.0, rx: None, ry: None })),
        style: NodeStyle::default(),
        transforms: vec![],
        children: vec![],
    };
    ...
    SvgRenderTree {
        root,
        viewport: ViewportInfo { width: 100.0, height: 100.0, view_box: None, overflow_visible: false, aspect_ratio: None },
        gradients: HashMap::new(), clip_paths: HashMap::new(),
        patterns: HashMap::new(), masks: HashMap::new(), filters: HashMap::new(),
    }
}
