...
try {
  ...
  if (issues.length) {
    ...
    process.exit(issues.length > 10 ? 1 : 0);
  }
  console.log('VALIDATION PASSED: ' + stats.totalNodes + ' nodes, ' + stats.totalEdges + ' edges, ' + stats.totalLayers + ' layers, ' + stats.tourSteps + ' tour steps');
  process.exit(0);
