...
2. [Pass 1 — Layout: source=None → Queue Serialization](#pass-1--layout-sourcenone--queue-serialization)
3. [Pass 1 → Pass 2 Bridge — Post-Reflow Serialization](#pass-1--pass-2-bridge--post-reflow-serialization)
4. [Pass 2 — Layout: Image Cache Wait](#pass-2--layout-image-cache-wait)
5. [Pass 2 → Pass 3 Bridge — VectorImage Loading Complete](#pass-2--pass-3-bridge--vectorimage-loading-complete)
6. [Pass 3 — Layout: VectorImage Available + Rasterization](#pass-3--layout-vectorimage-available--rasterization)
7. [Pass 3 → Pass 4 Bridge — Rasterization Complete](#pass-3--pass-4-bridge--rasterization-complete)
8. [Pass 4 — Layout: Fragment::Image → Display List](#pass-4--layout-fragmentimage--display-list)
9. [Reflow Trigger Summary](#9-reflow-trigger-summary)
10. [Key Values Table](#10-key-values-table)
...
## Pass 1 — Layout: source=None → Queue Serialization
...
| **Post-reflow state** | `pending_svg_elements_for_serialization` = `["b3c8d2f4-..."]` |

---

## Pass 1 → Pass 2 Bridge — Post-Reflow Serialization

**Location:** Within `Window::reflow()`, after layout returns
**Thread:** Script Thread
**`svg_kind_size()`:** Does NOT run here (this is post-reflow processing, not a layout pass)

### Stage 3: Serialization Dispatch

**Purpose:** Layout returns queued SVG node addresses to script via `ReflowResult`. The `Window::reflow()` method drains the list and dispatches serialization.

**Key function:**

| Function | File | Line | Purpose |
|----------|------|------|---------|
| `handle_pending_images_post_reflow()` | `components/script/dom/window.rs` | 3521 | Post-reflow hook; iterates pending SVG elements and serializes them |
...
handle_pending_images_post_reflow()    ← post-reflow hook
    ...
    └── dirty(NodeDamage::Other)       ← triggers next reflow
```

### Stage 4: Subtree Serialization

**Purpose:** Serialize the SVG subtree to XML, base64-encode it, cache as `data:image/svg+xml;base64,...` URL on the `SVGSVGElement`.
...
**Operations:**
1. `process_use_elements()` — scans for `<use>` (none in our test case, no-op)
...
6. Calls `dirty(NodeDamage::Other)` — this triggers the next rendering update
...
| Attribute | Value |
|-----------|-------|
| **Where** | `Window::reflow()` post-reflow hook (same call as Pass 1) |
| **`svg_kind_size()` runs?** | ❌ No — this is post-reflow, not a layout pass |
| **Key output** | `cached_serialized_data_url = Some(Ok(data:...))` |
| **Next trigger** | `dirty(NodeDamage::Other)` → `needs_rendering_update() = true` → schedule timer |
...
**Rendering Update:** 2
**Trigger:** `dirty(NodeDamage::Other)` set during bridge → timer fires → `update_the_rendering()` re-enters
...
#### 5.1: svg_kind_size() — P2 (Cache Wait)
...
2. `SVGElementData::data()` reads `source = Some(Ok(url))` — the data URL from Stage 4 serialization
...
#### 5.2: Async VectorImage Load (Image Cache Thread)
...
### Stage 5 Re-entry: Vector Cache Hit
...
### Stage 7: Cache Response (no-op same as Pass 3)
...
**Concrete values:** Identical to Pass 3's Stage 5.
...
| **Pass 1 → Bridge** | Post-reflow serialization | `handle_pending_images_post_reflow()` drains SVG list | `window.rs:3582-3591` |
| **Bridge → Pass 2** | `dirty(NodeDamage::Other)` | Sets restyle → `needs_rendering_update()` → schedule timer | `window.rs:3589` → `document.rs:2849` → `script_thread.rs:1302` |
...
Initial DOM Ready
    ...
    │           ├─ serialize_and_cache_subtree()  ← BRIDGE
